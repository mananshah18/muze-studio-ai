...
export function transformCode(code: string): string {
  ...
  transformedCode = transformedCode.replace(
    /const\s+Muze\s*=\s*env\.Muze/g,
    "const Muze = env"
  );

  // Replace viz references if needed
  transformedCode = transformedCode.replace(
    /const\s+{\s*muze\s*,\s*getDataFromSearchQuery\s*}\s*=\s*viz\s*;/g,
    "const { muze, getDataFromSearchQuery } = window.viz;"
  );
  transformedCode = transformedCode.replace(
    /const\s+{\s*muze\s*,\s*getDataFromSearchQuery\s*}\s*=\s*thoughtspot\s*;/g,
    "const { muze, getDataFromSearchQuery } = window.viz;"
  );
  ...
  transformedCode = transformedCode.replace(
    /thoughtspot\.getDataFromSearchQuery/g,
    "window.viz.getDataFromSearchQuery"
  );
  transformedCode = transformedCode.replace(
    /thoughtspot\.muze/g,
    "window.viz.muze"
  ...
  transformedCode = transformedCode.replace(
    /import\s+{\s*.*?\s*}\s+from\s+['"]@thoughtspot\/ts-chart-sdk['"]\s*;/g,
    "// ThoughtSpot SDK imports handled internally"
  );

  // Handle ChartContext initialization
  transformedCode = transformedCode.replace(
    /getChartContext\s*\(/g,
    "// getChartContext handled internally: "
  );

  // Ensure viz is properly referenced
  if (
    !transformedCode.includes("window.viz") &&
    !transformedCode.includes("const { muze, getDataFromSearchQuery }")
  ) {
